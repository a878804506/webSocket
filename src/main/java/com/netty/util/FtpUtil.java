...
import com.netty.common.Configuration;
...
public class FtpUtil {
    /** 日志对象 **/
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(FtpUtil.class);
    ...
    //ftp 主机名
    public static String ftp_hostname = Configuration.propMap.get("ftp.hostname");
    //ftp 端口
    public static int ftp_port = Integer.valueOf(Configuration.propMap.get("ftp.port"));
    //ftp 用户名
    public static String ftp_username = Configuration.propMap.get("ftp.username");
    //ftp 密码
    public static String ftp_password = Configuration.propMap.get("ftp.password");
    // messageFilePath     聊天和文件保存在ftp服务器的位置
    public static String messageFilePath = Configuration.propMap.get("ftp.messageFilePath");

    // ftp服务器
    public static String staticFileHost = Configuration.propMap.get("ftp.staticFileHost");

    // 项目temp 文件夹绝对路径
    public static final String localFilePath = "/"+FtpUtil.class.getResource("/").getPath().replaceFirst("/", "").replaceAll("WEB-INF/classes/", "temp");

    /**
     * 本地文件上传到FTP服务器
     *
     * @param ftpPath FTP服务器文件相对路径，例如：test/123
     * @param fileNames 上传到FTP服务的文件名集合 可一次上传多张图片，例如：666.txt
     * @return boolean 成功返回true，否则返回false
     */
    public static boolean uploadLocalFile(String ftpPath, List<String> fileNames) {
        // 登录
        login();
        boolean flag = false;
        if (ftpClient != null) {
            ...
            try {
                ...
                for ( String fileName : fileNames) {
                    File file = new File(localFilePath + System.getProperty("file.separator") + fileName);
                    fis = new FileInputStream(file);
                    ftpClient.storeFile(new String(fileName.getBytes(localCharset), serverCharset), fis);
                    fis.close();
                }
                flag = true;
            } catch (Exception e) {
...
//                closeConnect();
     ...
     * 删除指定文件
     *
     * @param filePath 文件相对路径，例如：test/123/
     * @param fileNames  要删除的文件集合
     * @return 成功返回true，否则返回false
     */
    public static boolean deleteFile(String filePath,List<String> fileNames) {
        // 登录
        login();
        boolean flag = false;
        if (ftpClient != null) {
